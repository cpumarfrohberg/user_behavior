...
async def evaluate_orchestrator_agent(
    ...
    for i, item in enumerate(ground_truth, 1):
        ...
        try:
            ...
            # Run judge evaluation (orchestrator-specific)
            # Note: Orchestrator doesn't expose tool_calls in result, so we pass None
            judge_result = await evaluate_orchestrator_answer(
                question,
                result.answer,
                tool_calls=None,  # Orchestrator doesn't track tool_calls in result
                expected_sources=expected_sources,
                judge_model=judge_model,
            )
            judge_score = judge_result.evaluation.overall_score
            ...
            results.append(
                {
                    ...
                    "num_tokens": total_tokens,
                    "combined_score": combined_score,
                    "agents_used": result.answer.agents_used,  # Orchestrator-specific
                }
            )
            ...
            logger.info(
                ...
                f"mrr={mrr:{score_format}}, judge_score={judge_score:{score_format}}, "
                f"combined_score={combined_score:{score_format}}, "
                f"agents_used={result.answer.agents_used}"
            )
            ...
            results.append(
                {
                    ...
                    "num_tokens": FALLBACK_NUM_TOKENS,
                    "combined_score": FALLBACK_COMBINED_SCORE,
                    "agents_used": [],  # Empty list for failed evaluations
                }
            )
    ...
    logger.info(f"Orchestrator evaluation complete. Results saved to: {output_path}")
