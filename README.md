...
# Neo4j (local; runs in Docker, use bolt://localhost:7687 from host)
NEO4J_URI=bolt://localhost:7687
NEO4J_USER=neo4j
NEO4J_PASSWORD=your_secure_password

# PostgreSQL (for monitoring; port 5433 when using docker-compose)
DATABASE_URL=postgresql://postgres:postgres@localhost:5433/user_behavior_monitoring
...
- Connect to PostgreSQL at `localhost:5433` for direct database access (when using Docker Compose)
